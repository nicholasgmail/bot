            ...
            <div class="overflow-hidden shadow-xl sm:rounded-lg">
                <div class="bg-brown text-white px-4 py-4">
                    <div class="max-w-xs mx-auto">
                        <input wire:model="name" class="{{$classes}}" placeholder="Игра">

                        @if($errors->isNotEmpty())
                            @error('name') <span class="font-bold text-red-500">{{ $message }}</span> @enderror
                        @else
                            <span>Ведди название игры от 3 до 64 символов</span>
                        @endif
                    </div>
                    <div class="text-center mt-4">
                        <button wire:click="addGame"
                                type="button"
                                ...
                                @foreach($allGame as $item)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{route('game_edit', $item->id)}}" class="text-green-600 hover:text-green-900">Редактировать</a>
                                            <span>|</span>
                                            <a href="#" wire:click.prevent="removeGame({{$item->id}})"  class="text-red-600 hover:text-red-900">Удалить</a>
                                        </td>
                                    </tr>
                                @endforeach
                            @else
