                            ...
                            @if($categories->isNotEmpty())
                                @foreach($categories as $item)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">{{$item->cell_number}}</div>
                                            {{--<div class="text-sm text-gray-500">Optimization</div>--}}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{route('category_edit', $item->id)}}" class="text-green-600 hover:text-green-900">Редактировать</a>
                                            <span>|</span>
                                            <a href="#" wire:click.prevent="removeCategory({{$item->id}})"  class="text-red-600 hover:text-red-900">Удалить</a>
                                        </td>
                                    </tr>
                                @endforeach
                            @else
